...
/*
    ...
    La dejo sin FOREIGN KEY para evitar errores por diferencias en tu BD.
*/
...
if (isset($usuario['nombre']) && $usuario['nombre'] !== '') {
    $nombreUsuario = $usuario['nombre'];
} elseif (isset($usuario['nombre_usuario']) && $usuario['nombre_usuario'] !== '') {
    $nombreUsuario = $usuario['nombre_usuario'];
} elseif (isset($usuario['usuario']) && $usuario['usuario'] !== '') {
    $nombreUsuario = $usuario['usuario'];
} elseif (isset($usuario['username']) && $usuario['username'] !== '') {
    $nombreUsuario = $usuario['username'];
} elseif (isset($usuario['email']) && $usuario['email'] !== '') {
    $nombreUsuario = $usuario['email'];
} else {
    $nombreUsuario = 'Jugador';
}

$saldo = isset($usuario['saldo']) ? (float) $usuario['saldo'] : 0;
    ...
    <div class="max-w-7xl mx-auto px-4 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div>
            ...
            <p class="text-zinc-400">
                Bienvenido, <?= escapar($nombreUsuario) ?>
            </p>
        </div>
        ...
        <div class="flex gap-3">
            ...
            <a href="logout.php" class="bg-red-600 hover:bg-red-500 px-5 py-3 rounded-xl font-black transition">
                Cerrar sesión
            </a>
        </div>
...
<main class="max-w-7xl mx-auto px-4 py-8 space-y-8">
    ...
    <section class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-zinc-900/90 border border-yellow-400/30 rounded-3xl p-6 card-glow">
            <p class="text-zinc-400 text-sm font-bold uppercase">
                Saldo en cuenta
            </p>

            <div class="text-4xl font-black text-yellow-400 mt-2">
                $<?= dinero($saldo) ?>
            </div>

            <p class="text-zinc-500 text-sm mt-2">
                No incluye dinero reservado en una sesión activa.
            </p>
        </div>

        <div class="bg-zinc-900/90 border border-green-400/30 rounded-3xl p-6 card-glow">
            <p class="text-zinc-400 text-sm font-bold uppercase">
                Balance neto histórico
            </p>

            <div class="text-4xl font-black mt-2 <?= $balanceNeto >= 0 ? 'text-green-400' : 'text-red-500' ?>">
        ...
        <div class="bg-zinc-900/90 border border-blue-400/30 rounded-3xl p-6 card-glow">
            <p class="text-zinc-400 text-sm font-bold uppercase">
                Total de apuestas
            </p>

            <div class="text-4xl font-black text-blue-400 mt-2">
                <?= $totalApuestas ?>
            </div>

            <p class="text-zinc-500 text-sm mt-2">
                Total apostado: $<?= dinero($totalApostado) ?>
            </p>
        </div>
    ...
    <section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <div class="bg-zinc-900/90 rounded-2xl p-5 border border-zinc-700">
                <p class="text-zinc-400 text-sm">
                    Ganadas
                </p>
    ...
    <section>
        ...
        <div class="bg-zinc-900/90 border border-zinc-700 rounded-3xl overflow-hidden table-scroll overflow-x-auto">
